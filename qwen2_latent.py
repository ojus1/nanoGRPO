...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Check if input_text is a list (batch mode)
        if isinstance(input_text, list):
            # Validate that all inputs in batch have consistent mode (all latent or all normal)
            has_latent = ["<bot><num_thoughts=" in txt for txt in input_text]
            if not all(has_latent) and not all(not x for x in has_latent):
                raise ValueError(
                    "Mixed generation modes in batch detected. All inputs must either use latent reasoning "
                    "or normal generation mode, not a mix of both."
                )
            # Get latent config from the first example (if it exists)
            latent_config = self._extract_latent_config(input_text[0])
        else:
    ...
    def _generate_with_embeds(
        ...
        **kwargs
        ...
        # IMPROVED: More robust attention mask handling
        if past_key_values is not None:
            # Get the sequence length from past key values
            past_seq_len = self._get_past_length(past_key_values)
            batch_size = inputs_embeds.shape[0]
            
            # Create a fresh attention mask that exactly matches the expected dimensions
            local_kwargs['attention_mask'] = torch.ones(
                (batch_size, past_seq_len + inputs_embeds.shape[1]),
                dtype=torch.long, 
                device=inputs_embeds.device
            )

        # Prefill Phase: Process the entire prompt
        outputs = self._prefill(inputs_embeds, past_key_values=past_key_values, **local_kwargs)
    ...
    def _inject_noise_to_latent(self, latent_token, noise_scale=0.0):
        ...
        Args:
            ...
            noise_scale (float): Scale of noise to add (0.0 means no noise)
            
        Returns:
            torch.Tensor: Latent token with added noise
        """
        if noise_scale <= 0.0:
            return latent_token
    ...
    def _get_past_length(self, past_key_values):
        ...
        Args:
            past_key_values: Past key values cache, which could be:
                - A tuple of tuples (transformer format) 
                - A custom cache object with get_seq_length method
                - None
        
        Returns:
            int: The sequence length represented in the cache, or 0 if None
        """
        if past_key_values is None:
            return 0
        
        # Handle tuple format (most common)
        ...
        # Handle cache objects with get_seq_length
